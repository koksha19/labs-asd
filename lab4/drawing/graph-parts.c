...
void drawStraight(HDC, int, int, int, int, int, double);

void drawEdges(HDC handle, int **coords, double **matrix, int isDirect, double k) {
    ...
    for (int i = 0; i < vertices; i++) {
        for (int j = 0; j < vertices; j++) {
            ...
            if(matrix[i][j] && i != j){
                ...
                } else if(y1 == y2 && abs(x1 - x2) > 150 && y1 == START) {
                    drawUpper(handle, x1, y1, x2, y2, isDirect);
                } else {
                    drawStraight(handle, x1, y1, x2, y2, isDirect, k);
                }
            }
        }
    }
...
void drawStraight(HDC handle, int x1, int y1, int x2, int y2, int isDirect, double k) {
    double angle, tg;
    int xx, yy;
    int lenx = abs(x1 - x2);
    int leny = abs(y1 - y2);
    ...
    if(y1 == y2) {
        ...
        if (lenx > leny) {
            ...
            yy = RADIUS / sqrt(1 + tg * tg);
            xx = yy * tg;
            if(y1 > y2 && isDirect && k == 0.715) {
                drawArrow(handle, 148, START + 10, START + 7);
            } else if(y1 < y2) {
                xx = 0;
                yy = 0;
                if (isDirect) drawArrow(handle, -148, START + 10, 393);
            }
        } else {
            tg = leny / lenx;
            angle = atan(tg) * 180 / 3.1416;
        ...
        if (isDirect) drawArrow(handle, angle + 180, x2 + xx, y2 + yy);
    }
...
void drawEdge(HDC handle) {
    int x1 = 250;
    int x2 = 100;
    int y = 100;
    MoveToEx(handle, x1, y, NULL);
    LineTo(handle, x2, y);
    drawArrow(handle, 180, x2 + RADIUS, y);
}